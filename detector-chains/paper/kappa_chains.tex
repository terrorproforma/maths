...
Equalizing is not an arbitrary choice: the terminal helper height must bind
at any optimum.

\begin{lemma}\label{lem:tight}
Fix the weights $d$ and let
$f(x)=\min\{\mathrm{dev}_2,\ldots,\mathrm{dev}_{k-1},\mathrm{term}_A\}$ and
$g(x)=\mathrm{term}_H$ as functions of the splits $x\in[0,1]^{k-1}$.  Then
every maximizer of $\min\{f,g\}$ satisfies $f(x)=g(x)$.
\end{lemma}
...
and similarly for $\mathrm{term}_A$; moreover each is \emph{strictly}
increasing in at least one coordinate.  In contrast
$g=\sum_{i<k}(1-x_i)+(1-x_k)$ is strictly decreasing in every coordinate.
Suppose $f(x)>g(x)$.  Then some $x_i>0$, since $x=0$ gives $g=k>1=f$;
decreasing that coordinate slightly raises $g$ and cannot increase $f$, so
$\min\{f,g\}=g$ strictly increases, contradicting optimality.  Suppose
$g(x)>f(x)$.  Then some $x_i<1$, since $x=\mathbf 1$ gives $g=0<f$;
increasing every coordinate below $1$ slightly raises all $A$-side heights
strictly, hence $f$, while $g$ decreases but retains slack for a small
enough step, so $\min\{f,g\}=f$ strictly increases.  Both cases being
impossible, $f=g$.
\end{proof}

\noindent
The balance solution \eqref{eq:balance} is the unique split vector making
\emph{all} $k$ heights equal, and it satisfies the conclusion of
Lemma~\ref{lem:tight}.  We do not prove that equalizing the $A$-side
heights among themselves is optimal; a linear program over the splits
confirms it at every weight vector we tested (agreement to nine decimals
at the certificate points of depths $4$, $5$, and $7$).  Nothing below
depends on this: any feasible design yields a lower bound on $\kappa$, so
the certificates stand regardless.
...
strongest assembled instance, and comes within $0.0040$ and $0.0021$ of
...
\item Is the balanced design optimal among all splits for fixed weights
(Lemma~\ref{lem:tight} gives only that the helper height binds), and is
the chain optimal among all single-block gadgets?  Our search
