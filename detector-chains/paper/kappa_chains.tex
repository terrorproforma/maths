...
Equalizing is not an arbitrary choice: the terminal helper height binds at
every optimum except in one boundary configuration, which we isolate
because a naive monotonicity argument overlooks it.
...
\emph{all} $k$ heights equal, and it satisfies the first alternative.  We
neither exclude the second alternative in general nor prove that
equalizing the $A$-side heights among themselves is optimal; a linear
program over the splits agrees with the balanced value at every weight
vector we tested, to nine decimals at the certificate points of depths
$4$, $5$ and $7$.  The direction of this residual uncertainty is worth
noting: the second alternative has value $1+d_2$, which is exactly the
admissibility cap of \eqref{eq:tau}, so were it ever attained the balanced
design would be \emph{understating} the chain's strength.  Every bound
below is safe either way, since any feasible design bounds $\kappa$ from
below.
